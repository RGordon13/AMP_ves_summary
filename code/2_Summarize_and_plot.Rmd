...
ggsave(paste0("figs/", "Transit_maneuver_counts_color_stripes_npz.png"), width=10, height=8,
       units="in", dpi=300)
...
### Plot mean-adjusted weekday counts -- bar plot

```{r bar-plot weekly mean adj, echo=FALSE}
...
ggplot(data = wkly_means_tot,
  ...
  geom_col(data = wkly_means_ins,
           aes(x = Weekday, 
               y = mean_wkdy_adj,
               fill = ifelse(mean_wkdy_adj >= 0, 'over','under')
           )
           )+
  ...
  scale_fill_manual(values = c("over" = "firebrick", 
                               "under" = "darkblue"))+
...
wkdy_plot_tempeast <- ggplot(data = wkly_means_tot |> 
  ...
  scale_fill_manual(values = c("over" = "firebrick1", 
                               "under" = "deepskyblue3",
                               "center" = "grey75"))+
  labs(y = "Mean-adjusted average N vessels / weekday",
       x = "Weekday")+
...
ggsave(paste0("figs/", "Weekday_mean_adj_horiz_panels_bars.png"), width=12, height=8, units="in", dpi=300)

```



### Weekday inside line plots (not using)

```{r weekday inside plot}
# Inside vessels
...
### Plot mean-adjusted weekday counts -- line plot

```{r line-plot weekly mean adj, echo=FALSE}
...
### Inside only diel plots (not using)
