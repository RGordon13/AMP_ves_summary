...
wkdy_mean_adj <- all_ves_wkdy |>
  ...
  filter(n_days == 7) |>
  mutate(tot_wkdy_adj = Total_ves_dep - mean_wk_tot)

ggplot(data = wkdy_mean_adj |> filter(network.x == "South-west"),
       aes(x = Weekday, y = tot_wkdy_adj)) +
  geom_boxplot(aes(fill = network.x)) +
...
## Try mean-adjusted diel presence
