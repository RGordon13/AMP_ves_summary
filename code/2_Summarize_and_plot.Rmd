...
gaps <- gaps |>
  mutate(network = case_when(npz_id == "CGMP" | npz_id == "SIMP" ~ "Temperate East",
                             npz_id == "DMP" | npz_id == "NMP" ~ "North-west", TRUE ~ "South-west"
         ),
         network = factor(network, levels = c("Temperate East","North-west","South-west")))
...
```{r save div bar fig}
ggsave(paste0("figs/", "Ins_out_counts_npz_colors_gray18.png"), width=12, height=11,
       units="in", dpi=300)
...
ggsave(paste0("figs/", "Transit_maneuver_counts_color_npz18.png"), width=12, height=11,
       units="in", dpi=300)
...
inout_freq_table <- all_ves_long |>
  ...
  group_by(network, npz_dep, Ves_behav) |> 
...
# Inside vessels: Temperate East network


inout_tmpeast <- inout_freq_table |> filter(network=="Temperate East")

fishtest_tmpeast <- fisher.test(inout_tmpeast[3:4], simulate.p.value = TRUE)
# data:  inout_freq_table[2:3]
# p-value = 0.0004998
# alternative hypothesis: two.sided

ins_tmpeast <- inout_tmpeast$Total_inside
names(ins_tmpeast) <- inout_tmpeast$npz_dep
tot_tmpeast <- inout_tmpeast$total_vessels
names(tot_tmpeast) <- inout_tmpeast$npz_dep


# Proportion inside: Post hoc pairwise comparisons with Bonferonni correction 
pairwise.fisher.test(ins_tmpeast, tot_tmpeast, p.adjust.method = "bonf")

# 	Pairwise comparisons using Pairwise comparison of proportions (Fisher) 
# 
# data:  ins_tmpeast out of tot_tmpeast 
# 
#            CGMP201807 CGMP201812 CGMP201904 SIMP201808 SIMP201812
# CGMP201812 1.00000    -          -          -          -         
# CGMP201904 1.00000    1.00000    -          -          -         
# SIMP201808 0.01878    0.00034    0.00098    -          -         
# SIMP201812 1.00000    1.00000    1.00000    0.00029    -         
# SIMP201906 0.10207    0.00102    0.00283    1.00000    0.00083   
# 
# P value adjustment method: bonferroni 



# Inside vessels: North-west network
# NS overall test, no pairwise post hoc


inout_northwest <- inout_freq_table |> filter(network=="Northwest")

fishtest_northwest <- fisher.test(inout_northwest[3:4], simulate.p.value = TRUE)

# data:  inout_northwest[3:4]
# p-value = 0.5499
...
# Inside vessels: South-west network


inout_swest <- inout_freq_table |> filter(network=="South-west")

fishtest_swest <- fisher.test(inout_swest[3:4], simulate.p.value = TRUE)
# data:  inout_freq_table[2:3]
# p-value = 0.0004998
# alternative hypothesis: two.sided

ins_swest <- inout_swest$Total_inside
names(ins_swest) <- inout_swest$npz_dep
tot_swest <- inout_swest$total_vessels
names(tot_swest) <- inout_swest$npz_dep


# Proportion inside: Post hoc pairwise comparisons with Bonferonni correction 
pairwise.fisher.test(ins_swest, tot_swest, p.adjust.method = "bonf")
 
# 	Pairwise comparisons using Pairwise comparison of proportions (Fisher) 
# 
# data:  ins_swest out of tot_swest 
# 
#             GMP202201 JMP202201 MMP202002 SWCMP202201
# JMP202201   1.8e-05   -         -         -          
# MMP202002   1         1         -         -          
# SWCMP202201 1         5.5e-05   1         -          
# TRMP202105  1.3e-08   1         1         3.8e-07    
# 
# P value adjustment method: bonferroni 
...
# Maneuvers -- Temperate East
man_freq_tmpeast <- man_freq_table |> filter(network=="Temperate East")

fishtest_tmpeast_man <- fisher.test(man_freq_tmpeast[3:4], simulate.p.value = TRUE)
# data:  man_freq_tmpeast[3:4]
# p-value = 0.0004998
# alternative hypothesis: two.sided

man_tmpeast <- man_freq_tmpeast$Inside_maneuver
names(man_tmpeast) <- man_freq_tmpeast$npz_dep
tot_ins_tmpeast <- man_freq_tmpeast$total_inside
names(tot_ins_tmpeast) <- man_freq_tmpeast$npz_dep


# Proportion inside: Post hoc pairwise comparisons with Bonferonni correction 
pairwise.fisher.test(man_tmpeast, tot_ins_tmpeast, p.adjust.method = "bonf")

# 	Pairwise comparisons using Pairwise comparison of proportions (Fisher) 
# 
# data:  man_tmpeast out of tot_ins_tmpeast 
# 
#            CGMP201807 CGMP201812 CGMP201904 SIMP201808 SIMP201812
# CGMP201812 0.28078    -          -          -          -         
# CGMP201904 0.64358    1.00000    -          -          -         
# SIMP201808 1.00000    0.03192    0.11011    -          -         
# SIMP201812 0.03005    1.00000    1.00000    0.00255    -         
# SIMP201906 2.0e-08    0.06055    0.00028    1.9e-10    0.14053   
# 
# P value adjustment method: bonferroni 
...
man_freq_northwest <- man_freq_table |> filter(network=="Northwest")

fishtest_northwest_man <- fisher.test(man_freq_northwest[3:4], simulate.p.value = TRUE)
# data:  man_freq_northwest[3:4]
# p-value = 0.0004998
# alternative hypothesis: two.sided


# Maneuvers -- South-west
man_freq_swest <- man_freq_table |> 
  filter(network=="South-west" & total_inside > 0)

fishtest_swest_man <- fisher.test(man_freq_swest[3:4], simulate.p.value = TRUE)
# data:  man_freq_swest[3:4]
# p-value = 0.0004998
# alternative hypothesis: two.sided

man_swest <- man_freq_swest$Inside_maneuver
names(man_swest) <- man_freq_swest$npz_dep
tot_ins_swest <- man_freq_swest$total_inside
names(tot_ins_swest) <- man_freq_swest$npz_dep


# Proportion inside: Post hoc pairwise comparisons with Bonferonni correction 
pairwise.fisher.test(man_swest, tot_ins_swest, p.adjust.method = "bonf")
# data:  man_swest out of tot_ins_swest 
# 
#            GMP202201 JMP202201
# JMP202201  1.00      -        
# TRMP202105 0.48      2.1e-08  
...
# use named vectors to preserve NPZ ids in output
ins_man <- maneuver_freq_table$Inside_maneuver
names(ins_man) <- maneuver_freq_table$npz_dep
ins_tot <- maneuver_freq_table$total_inside
names(ins_tot) <- maneuver_freq_table$npz_dep
...
wkly_mean_adj_tot_tmpeast <- wkly_mean_adj_tot |>
  filter(network == "Temperate East") |>
  mutate(npz_dep = as_factor(paste0(npz_id, Dep_ID)))

# deployment*Weekday
week_aov_tmpeast <- aov(tot_wkdy_adj ~ npz_dep*Weekday, 
                data = wkly_mean_adj_tot_tmpeast)

summary(week_aov_tmpeast)

# sanity check about npz_dep main effect
ggplot(data = wkly_mean_adj_tot_tmpeast, aes(x = npz_dep, y = tot_wkdy_adj)) + 
...
tukey_week_tmpeast <- tidy(TukeyHSD(week_aov_tmpeast)) |>
  filter(adj.p.value <= 0.05)



# total counts -- Northwest

wkly_mean_adj_tot_northwest <- wkly_mean_adj_tot |>
  filter(network == "Northwest") |>
  mutate(npz_dep = as_factor(paste0(npz_id, Dep_ID)))

# deployment*Weekday
week_aov_northwest <- aov(tot_wkdy_adj ~ npz_dep*Weekday, 
                data = wkly_mean_adj_tot_northwest)

summary(week_aov_northwest)

# sanity check about npz_dep main effect
ggplot(data = wkly_mean_adj_tot_northwest, aes(x = npz_dep, y = tot_wkdy_adj)) + 
  geom_point(position = position_jitter(width = 0.1)) + 
  geom_boxplot(alpha = 0.8)
# makes sense that site contributed no variation since scores are all centered around 0
...
tukey_week_northwest <- tidy(TukeyHSD(week_aov_northwest)) |>
  filter(adj.p.value <= 0.05)



# total counts -- Southwest

wkly_mean_adj_tot_swest <- wkly_mean_adj_tot |>
  filter(network == "South-west") |>
  mutate(npz_dep = as_factor(paste0(npz_id, Dep_ID)))

# deployment*Weekday
week_aov_swest <- aov(tot_wkdy_adj ~ npz_dep*Weekday, 
                data = wkly_mean_adj_tot_swest)

summary(week_aov_swest)

# sanity check about npz_dep main effect
ggplot(data = wkly_mean_adj_tot_swest, aes(x = npz_dep, y = tot_wkdy_adj)) + 
  geom_point(position = position_jitter(width = 0.1)) + 
  geom_boxplot(alpha = 0.8)
# makes sense that site contributed no variation since scores are all centered around 0
...
tukey_week_swest <- tidy(TukeyHSD(week_aov_swest)) |>
  filter(adj.p.value <= 0.05)
...
# Inside -- Temperate east

wkly_mean_adj_ins_tmpeast <- wkly_mean_adj_ins |>
  filter(network == "Temperate East") |>
  mutate(npz_dep = as_factor(paste0(npz_id, Dep_ID)))

# 2-way ANOVA deployment*Weekday
week_ins_aov_tmpeast <- aov(tot_wkdy_adj ~ npz_dep*Weekday, 
                data = wkly_mean_adj_ins_tmpeast)
...
tukey_week_ins <- tidy(TukeyHSD(week_ins_aov_tmpeast)) |>
  filter(adj.p.value <= 0.05)


# Inside -- Northwest

wkly_mean_adj_ins_northwest <- wkly_mean_adj_ins |>
  filter(network == "Northwest") |>
  mutate(npz_dep = as_factor(paste0(npz_id, Dep_ID)))

# 2-way ANOVA deployment*Weekday
week_ins_aov_northwest <- aov(tot_wkdy_adj ~ npz_dep*Weekday, 
                data = wkly_mean_adj_ins_northwest)

summary(week_ins_aov_northwest)
## NS overall test, no post hoc Tukey



# Inside -- South-west

wkly_mean_adj_ins_swest <- wkly_mean_adj_ins |>
  filter(network == "South-west") |>
  mutate(npz_dep = as_factor(paste0(npz_id, Dep_ID)))

# 2-way ANOVA deployment*Weekday
week_ins_aov_swest <- aov(tot_wkdy_adj ~ npz_dep*Weekday, 
                data = wkly_mean_adj_ins_swest)

summary(week_ins_aov_swest)
# NS overall test, no post hoc
...
# Maneuvers -- Temperate East
wkly_mean_adj_ins_man_tmpeast <- wkly_mean_adj_ins_man |>
    filter(network == "Temperate East") |>
  mutate(npz_dep = as_factor(paste0(npz_id, Dep_ID)))

# 2-way ANOVA deployment*Weekday
week_man_aov_tmpeast <- aov(man_wkdy_adj ~ npz_dep*Weekday, 
                data = wkly_mean_adj_ins_man_tmpeast)
...
# Maneuvers -- Northwest
wkly_mean_adj_ins_man_northwest <- wkly_mean_adj_ins_man |>
    filter(network == "Northwest") |>
  mutate(npz_dep = as_factor(paste0(npz_id, Dep_ID)))

# 2-way ANOVA deployment*Weekday
week_man_aov_northwest <- aov(man_wkdy_adj ~ npz_dep*Weekday, 
                data = wkly_mean_adj_ins_man_northwest)

summary(week_man_aov_northwest)

# Tukey HSD for interaction model

tukey_week_man_northwest <- tidy(TukeyHSD(week_man_aov_northwest)) |>
  filter(adj.p.value <= 0.05)



# Maneuvers -- South-west
wkly_mean_adj_ins_man_swest <- wkly_mean_adj_ins_man |>
    filter(network == "South-west") |>
  mutate(npz_dep = as_factor(paste0(npz_id, Dep_ID)))

# 2-way ANOVA deployment*Weekday
week_man_aov_swest <- aov(man_wkdy_adj ~ npz_dep*Weekday, 
                data = wkly_mean_adj_ins_man_swest)

summary(week_man_aov_swest)

# Tukey HSD for interaction model

tukey_week_man_swest <- tidy(TukeyHSD(week_man_aov_swest)) |>
  filter(adj.p.value <= 0.05)
...
ggarrange(wkdy_plot_tempeast, wkdy_plot_northwest, wkdy_plot_southwest,
          nrow = 1, 
          labels = c("Temperate East","North-west","South-west"),
          vjust=1.25)
```
...
ggsave(paste0("figs/", "Weekday_mean_adj_horiz_panels_bars_maneuver_free_y.png"), width=12, height=8, units="in", dpi=300)
...
# Daily mean-adjusted presence by light regime
...
# Calculate total daily mean-adjusted counts per light regime
...
# Calculate daily mean-adjusted counts per hour for inside vessels
light_mean_adj_ins <- all_ves_light |>
  # group to date level
  group_by(network, npz_id, Dep_ID, Dep, Begin_Date_loc) |>
  # get n light conditions per day
  summarize(n_hrs = n(),
            # and inside vessels per day
            day_sum = sum(Total_inside, na.rm = TRUE)) |>
  # only keep days with all light conditions == 4 (dawn/day/dusk/night)
  # and days with vessels present
  filter(n_hrs == 4,
  ...
  mutate(mean_tot = mean(Total_inside, na.rm = TRUE),
         # calculate mean-adjusted counts per light regime
         tot_light_adj = Total_inside - mean_tot) 
 
 
 # summarize means & SD for plots
 light_means_ins <- light_mean_adj_ins |>
  # group to light level per deployment
  group_by(network, npz_id, Dep_ID, light_reg) |>
  # get mean of mean-adjusted ounts per hour
  summarise(mean_light_adj = mean(tot_light_adj, na.rm =TRUE)) |>
...
ggplot(data = light_mean_adj_ins,
        aes(x = npz_id, y = tot_light_adj, color = light_reg)) +
   geom_boxplot()
 
# Calculate daily mean-adjusted counts per hour for maneuvering vessels
light_mean_adj_man <- all_ves_light |>
  # group to date level
  group_by(network, npz_id, Dep_ID, Dep, Begin_Date_loc) |>
  # get n light conditions per day
  summarize(n_hrs = n(),
  ...
  group_by(network, npz_id, Dep_ID, Dep, Begin_Date_loc) |>
  # calculate mean total presence per day with vessels
  mutate(mean_tot = mean(Inside_maneuver, na.rm = TRUE),
         # calculate mean-adjusted counts per light regime
         tot_light_adj = Inside_maneuver - mean_tot) 
 
 
 # summarize means & SD for plots
 light_means_man <- light_mean_adj_man |>
  # group to light level per deployment
  group_by(network, npz_id, Dep_ID, light_reg) |>
  # get mean of mean-adjusted ounts per hour
  summarise(mean_light_adj = mean(tot_light_adj, na.rm =TRUE)) |>
...
ggplot(data = light_mean_adj_man,
        aes(x = npz_id, y = tot_light_adj, color = light_reg)) +
   geom_boxplot() 
...
# Kruskal-Wallis for diel patterns
```{r kw diel tests}
...
# total vessels -- Temperate East
kruskal.test(tot_light_adj ~ factor(light_reg), 
                data = light_mean_adj_tot |> 
               filter(network == "Temperate East"))
# Kruskal-Wallis chi-squared = 695.59, df = 3, p-value < 2.2e-16


dunn_light_tot_tmpeast <- dunnTest(tot_light_adj ~ factor(light_reg), 
                          data = light_mean_adj_tot |> 
                            filter(network == "Temperate East"),
                          method = "holm")

dunntable_light_tot_tmpeast <- as.data.frame(dunn_light_tot_tmpeast$res) |>
  filter(P.adj <= 0.05)



# total vessels -- Northwest
kruskal.test(tot_light_adj ~ factor(light_reg), 
                data = light_mean_adj_tot |> 
               filter(network == "Northwest"))
# Kruskal-Wallis chi-squared = 210.25, df = 3, p-value < 2.2e-16


dunn_light_tot_northwest <- dunnTest(tot_light_adj ~ factor(light_reg), 
                          data = light_mean_adj_tot |> 
                            filter(network == "Northwest"),
                          method = "holm")

dunntable_light_tot_northwest <- as.data.frame(dunn_light_tot_northwest$res) |>
  filter(P.adj <= 0.05)




# total vessels -- South-west
kruskal.test(tot_light_adj ~ factor(light_reg), 
                data = light_mean_adj_tot |> 
               filter(network == "South-west"))
# Kruskal-Wallis chi-squared = 740.34, df = 3, p-value < 2.2e-16


dunn_light_tot_swest <- dunnTest(tot_light_adj ~ factor(light_reg), 
                          data = light_mean_adj_tot |> 
                            filter(network == "South-west"),
                          method = "holm")

dunntable_light_tot_swest <- as.data.frame(dunn_light_tot_swest$res) |>
  filter(P.adj <= 0.05)

#'----------------------------------------------------------

# inside vessels -- Temperate East
kruskal.test(tot_light_adj ~ factor(light_reg), 
                data = light_mean_adj_ins |> 
               filter(network == "Temperate East"))
# Kruskal-Wallis chi-squared = 232.19, df = 3, p-value < 2.2e-16


dunn_light_ins_tmpeast <- dunnTest(tot_light_adj ~ factor(light_reg), 
                          data = light_mean_adj_ins |> 
                            filter(network == "Temperate East"),
                          method = "holm")

dunntable_light_ins_tmpeast <- as.data.frame(dunn_light_ins_tmpeast$res) |>
  filter(P.adj <= 0.05)



# inside vessels -- Northwest
kruskal.test(tot_light_adj ~ factor(light_reg), 
                data = light_mean_adj_ins |> 
               filter(network == "Northwest"))
# Kruskal-Wallis chi-squared = 7.6667, df = 3, p-value = 0.05343

# NS overall, no post hoc




# inside vessels -- South-west
kruskal.test(tot_light_adj ~ factor(light_reg), 
                data = light_mean_adj_ins |> 
               filter(network == "South-west"))
# Kruskal-Wallis chi-squared = 96.914, df = 3, p-value < 2.2e-16


dunn_light_ins_swest <- dunnTest(tot_light_adj ~ factor(light_reg), 
                          data = light_mean_adj_ins |> 
                            filter(network == "South-west"),
                          method = "holm")

dunntable_light_ins_swest <- as.data.frame(dunn_light_ins_swest$res) |>
  filter(P.adj <= 0.05)

#'----------------------------------------------------------

# maneuverss -- Temperate East
kruskal.test(tot_light_adj ~ factor(light_reg), 
                data = light_mean_adj_man |> 
               filter(network == "Temperate East"))
# Kruskal-Wallis chi-squared = 166.06, df = 3, p-value < 2.2e-16


dunn_light_man_tmpeast <- dunnTest(tot_light_adj ~ factor(light_reg), 
                          data = light_mean_adj_man |> 
                            filter(network == "Temperate East"),
                          method = "holm")

dunntable_light_man_tmpeast <- as.data.frame(dunn_light_man_tmpeast$res) |>
  filter(P.adj <= 0.05)
...
kruskal.test(tot_light_adj ~ factor(light_reg), 
                data = light_mean_adj_man |> 
               filter(network == "Northwest"))
# Kruskal-Wallis chi-squared = 7, df = 3, p-value = 0.0719
...
kruskal.test(tot_light_adj ~ factor(light_reg), 
                data = light_mean_adj_man |> 
               filter(network == "South-west"))
# Kruskal-Wallis chi-squared = 62.931, df = 3, p-value = 1.389e-13


dunn_light_man_swest <- dunnTest(tot_light_adj ~ factor(light_reg), 
                          data = light_mean_adj_man |> 
                            filter(network == "South-west"),
                          method = "holm")

dunntable_light_man_swest <- as.data.frame(dunn_light_ins_swest$res) |>
  filter(P.adj <= 0.05)
...
kruskal.test(tot_diel_adj ~ factor(Begin_Hour_loc), 
               ...
               filter(network == "Temperate East"))
# Kruskal-Wallis chi-squared = 1476.8, df = 23, p-value < 2.2e-16


dunn_hour_tot_tmpeast <- dunnTest(tot_diel_adj ~ factor(Begin_Hour_loc), 
                          data = daily_mean_adj_tot |> 
                            filter(network == "Temperate East"),
                          method = "holm")

dunntable_tot_tmpeast <- as.data.frame(dunn_hour_tot_tmpeast$res) |>
  filter(P.adj <= 0.05)
...
kruskal.test(tot_diel_adj ~ factor(Begin_Hour_loc), 
                data = daily_mean_adj_tot |> 
               filter(network == "Northwest"))
# Kruskal-Wallis chi-squared = 65.81, df = 23, p-value = 5.299e-06


dunn_hour_tot_northwest <- dunnTest(tot_diel_adj ~ factor(Begin_Hour_loc), 
                          data = daily_mean_adj_tot |> 
                            filter(network == "Northwest"),
                          method = "holm")

dunntable_tot_northwest <- as.data.frame(dunn_hour_tot_northwest$res) |>
  filter(P.adj <= 0.05)
...
kruskal.test(tot_diel_adj ~ factor(Begin_Hour_loc), 
                data = daily_mean_adj_tot |> 
               filter(network == "South-west"))
# Kruskal-Wallis chi-squared = 673.35, df = 23, p-value < 2.2e-16


dunn_hour_tot_swest <- dunnTest(tot_diel_adj ~ factor(Begin_Hour_loc), 
                          data = daily_mean_adj_tot |> 
                            filter(network == "South-west"),
                          method = "holm")

dunntable_tot_swest <- as.data.frame(dunn_hour_tot_swest$res) |>
  filter(P.adj <= 0.05)
...
kruskal.test(ins_diel_adj ~ factor(Begin_Hour_loc), 
               ...
               filter(network == "Temperate East"))
# Kruskal-Wallis chi-squared = 84.899, df = 23, p-value = 5.022e-09


dunn_hour_ins_tmpeast <- dunnTest(ins_diel_adj ~ factor(Begin_Hour_loc), 
                          data = daily_mean_adj_ins |> 
                            filter(network == "Temperate East"),
                          method = "holm")

dunntable_ins_tmpeast <- as.data.frame(dunn_hour_ins_tmpeast$res) |>
  filter(P.adj <= 0.05)
...
kruskal.test(ins_diel_adj ~ factor(Begin_Hour_loc), 
                data = daily_mean_adj_ins |> 
               filter(network == "Northwest"))
# Kruskal-Wallis chi-squared = 27.122, df = 23, p-value = 0.2508
...
kruskal.test(ins_diel_adj ~ factor(Begin_Hour_loc), 
                data = daily_mean_adj_ins |> 
               filter(network == "South-west"))
# Kruskal-Wallis chi-squared = 38.506, df = 23, p-value = 0.02246


dunn_hour_ins_swest <- dunnTest(ins_diel_adj ~ factor(Begin_Hour_loc), 
                          data = daily_mean_adj_ins |> 
                            filter(network == "South-west"),
                          method = "holm")

dunntable_ins_swest <- as.data.frame(dunn_hour_ins_swest$res) |>
  filter(P.adj <= 0.05)
...
kruskal.test(ins_diel_man_adj ~ factor(Begin_Hour_loc), 
               ...
               filter(network == "Temperate East"))
# Kruskal-Wallis chi-squared = 57.838, df = 23, p-value = 7.798e-05


dunn_hour_man_tmpeast <- dunnTest(ins_diel_man_adj ~ factor(Begin_Hour_loc), 
                          data = daily_mean_adj_ins_man |> 
                            filter(network == "Temperate East"),
                          method = "holm")

dunntable_man_tmpeast <- as.data.frame(dunn_hour_man_tmpeast$res) |>
  filter(P.adj <= 0.05)
# all pairwise NS
...
kruskal.test(ins_diel_man_adj ~ factor(Begin_Hour_loc), 
                data = daily_mean_adj_ins_man |> 
               filter(network == "Northwest"))
# Kruskal-Wallis chi-squared = 29.595, df = 23, p-value = 0.1614
...
kruskal.test(ins_diel_man_adj ~ factor(Begin_Hour_loc), 
                data = daily_mean_adj_ins_man |> 
               filter(network == "South-west"))
# Kruskal-Wallis chi-squared = 41.468, df = 23, p-value = 0.01046


dunn_hour_man_swest <- dunnTest(ins_diel_man_adj ~ factor(Begin_Hour_loc), 
                          data = daily_mean_adj_ins_man |> 
                            filter(network == "South-west"),
                          method = "holm")

dunntable_man_swest <- as.data.frame(dunn_hour_man_swest$res) |>
  filter(P.adj <= 0.05)
...
# 1   dawn - day -33.369157 3.842517e-244 1.921258e-243*
# 2  dawn - dusk   2.125593  3.353720e-02  3.353720e-02*
# 3   day - dusk  35.494749 5.923338e-276 3.554003e-275*
# 4 dawn - night  -9.584905  9.254284e-22  1.850857e-21*
# 5  day - night  23.784252 4.860984e-125 1.944394e-124*
# 6 dusk - night -11.710497  1.126170e-31  3.378509e-31*

# all time periods sig different from one another for total vessels
# Day sig. greater than all others
# Night sig. greater than dusk & dawn
# Dawn sig. greater than dusk
...
# data:  tot_light_adj by light_reg
# Kruskal-Wallis chi-squared = 335.97, df = 3, p-value < 2.2e-16
...
#     Comparison           Z      P.unadj        P.adj
# 1   dawn - day -15.2228933 2.492591e-52 1.246296e-51*
# 2  dawn - dusk   0.3144364 7.531896e-01 7.531896e-01
# 3   day - dusk  15.5373297 1.938958e-54 1.163375e-53*
# 4 dawn - night  -1.2748278 2.023702e-01 4.047404e-01
# 5  day - night  13.9480655 3.232546e-44 1.293018e-43*
# 6 dusk - night  -1.5892642 1.120008e-01 3.360023e-01

# Day sig. higher than all others
...
# data:  tot_light_adj by light_reg
# Kruskal-Wallis chi-squared = 233.62, df = 3, p-value < 2.2e-16
...
#     Comparison            Z      P.unadj        P.adj
# 1   dawn - day -12.34488565 5.190794e-35 2.595397e-34*  
# 2  dawn - dusk   0.39042855 6.962197e-01 1.000000e+00
# 3   day - dusk  12.73531420 3.763280e-37 2.257968e-36*  
# 4 dawn - night  -0.00171348 9.986328e-01 9.986328e-01
# 5  day - night  12.34317217 5.302482e-35 2.120993e-34*  
# 6 dusk - night  -0.39214203 6.949533e-01 1.000000e+00
